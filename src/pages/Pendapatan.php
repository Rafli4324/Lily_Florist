...
<head>
  ...
  <title>Pendapatan - Lily Florist</title>
...
<body class="antialiased font-sans text-stone-700 bg-white overflow-hidden">
  ...
  <div class="flex h-screen">
      ...
      <div class="px-6 py-8">
        ...
        <nav class="mt-8">
          ...
          <div class="mt-6 text-sm">
            ...
            <a class="flex items-center bg-[#eecfd0] gap-3 px-3 py-2 rounded-md hover:bg-[#efdcdd]" href="Pendapatan.php">
            ...
            <a class="flex items-center gap-3 px-3 py-2 hover:bg-[#efdcdd] rounded-md mt-2" href="Pengeluaran.php">
    ...
    <div class="flex-1 flex flex-col min-w-0 bg-white">
      ...
      <main class="p-8 overflow-y-auto h-full">
        <div class="w-full">
          <h1 class="text-2xl font-bold text-[#4a3b3b] mb-6">Pendapatan</h1>

          <div class="bg-white p-0 rounded-none">
            <div class="flex flex-col sm:flex-row sm:items-end justify-between mb-4 gap-4">
              <div class="text-sm text-stone-600">
                <div class="font-bold text-stone-700 mb-1">Transaksi Masuk</div>
                <div class="flex items-center gap-2">
                  <span>Show</span>
                  <select id="entriesSelect" class="border border-stone-300 rounded px-2 py-1 text-sm focus:outline-none focus:border-stone-400">
              ...
              <button id="addIncomeBtn" class="btn-pink px-4 py-2 rounded shadow-sm text-sm font-medium flex items-center gap-2">
                <span>+</span> Pemasukan
              </button>
            </div>
            ...
            <div class="overflow-x-auto border border-[#e5d5d5] rounded-t-md">
              <table class="w-full text-sm text-left">
                ...
                <tbody id="incomeBody" class="bg-[#fcfaf9] divide-y divide-[#efe6e6]">
                </tbody>
              </table>
            </div>

            <div class="flex items-center justify-between mt-4">
                <div id="showingText" class="text-xs text-stone-500">Showing 0 to 0 of 0 entries</div>
  ...
  <div id="incomeModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/50 backdrop-blur-sm px-4">
    <div class="bg-white w-full max-w-lg rounded-lg shadow-2xl transform scale-100 transition-all">
      <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200">
        <h3 class="text-lg font-bold text-gray-700">Tambah Pendapatan</h3>
        <button id="incomeModalClose" class="text-gray-400 hover:text-gray-600 transition">
          <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
        </button>
      </div>

      <form id="incomeForm" class="px-6 py-6 space-y-5">
        
        <div>
          <label class="block text-sm font-medium text-gray-600 mb-1">Tanggal:</label>
          <div class="relative">
            <input id="incomeDate" type="date" class="w-full border border-gray-300 rounded-md px-3 py-2 text-sm text-gray-700 focus:outline-none focus:ring-1 focus:ring-[#d9a4a7] focus:border-[#d9a4a7]" />
          </div>
        </div>

        <div>
          <label class="block text-sm font-medium text-gray-600 mb-1">Jumlah:</label>
          <input id="incomeAmount" type="text" class="w-full border border-gray-300 rounded-md px-3 py-2 text-sm text-gray-700 focus:outline-none focus:ring-1 focus:ring-[#d9a4a7] focus:border-[#d9a4a7]" placeholder="3.000.000,00" />
        </div>

        <div>
          <label class="block text-sm font-medium text-gray-600 mb-1">Keterangan:</label>
          <textarea id="incomeNote" rows="3" class="w-full border border-gray-300 rounded-md px-3 py-2 text-sm text-gray-700 focus:outline-none focus:ring-1 focus:ring-[#d9a4a7] focus:border-[#d9a4a7]" placeholder="Hari ini laku..."></textarea>
        </div>

        <div class="flex justify-end gap-3 pt-4">
          <button type="button" id="incomeCancel" class="px-4 py-2 rounded-md text-sm font-medium bg-gray-200 text-gray-700 hover:bg-gray-300 transition">Kembali</button>
  ...
  <div id="incomeSuccess" class="fixed inset-0 z-60 hidden items-center justify-center bg-black/50 backdrop-blur-sm px-4">
    <div class="bg-white w-80 rounded-lg shadow-2xl p-8 text-center transform transition-all scale-100">
      ...
      <h3 class="text-base font-medium text-gray-800 mb-1">Pemasukan Berhasil</h3>
      <p class="text-base font-medium text-gray-800 mb-6">dicatat</p>

      <button id="incomeSuccessOk" class="btn-pink px-8 py-2 rounded shadow-sm text-sm font-medium">Oke</button>
    </div>
  </div>
...
<script>
  ...
  const modalClose = document.getElementById('incomeModalClose');
  const modalCancel = document.getElementById('incomeCancel');
  const modalForm = document.getElementById('incomeForm');
  const successModal = document.getElementById('incomeSuccess');
  const successOk = document.getElementById('incomeSuccessOk');
  ...
  function initializeSupabase() {
    ...
    console.log('Supabase client initialized for Income.');
    return true;
  }
      ...
      if(data.length === 0){
        for(let i=0; i < itemsPerPage; i++){
          ...
          placeholderRow.innerHTML = `
            ...
            <td></td>
          `;
          incomeBody.appendChild(placeholderRow);
        }
      } else {
          ...
          newRow.innerHTML = `
            ...
            <td class="px-6 py-4 text-center">
              <div class="flex justify-center gap-2">
                <button class="edit-income-btn bg-yellow-400 hover:bg-yellow-500 text-white text-[10px] px-3 py-1 rounded shadow-sm font-bold tracking-wide" data-id="${item.pendapatan_id}">Edit</button>
                <button class="delete-income-btn bg-red-500 hover:bg-red-600 text-white text-[10px] px-3 py-1 rounded shadow-sm font-bold tracking-wide" data-id="${item.pendapatan_id}">Hapus</button>
              </div>
            </td>
          `;
          incomeBody.appendChild(newRow);
        });
      }
  ...
  async function openEditModal(id) {
    ...
    try {
      ...
      document.getElementById('incomeDate').value = data.tanggal || '';
      document.getElementById('incomeAmount').value = data.jumlah ? formatRupiah(data.jumlah.toString()) : '';
      document.getElementById('incomeNote').value = data.keterangan || '';
  ...
  incomeBody.addEventListener('click', (e) => {
    ...
    } else if (e.target.classList.contains('edit-income-btn')) {
  ...
  modalForm.addEventListener('submit', async (e) => {
    ...
    try {
      if(isEditMode && editId){
        ...
        const { error } = await supabaseClient
          .from('tabel_pendapatan')
          .insert([{ tanggal: dateVal, jumlah: parseInt(amountVal), keterangan: noteVal }]);
